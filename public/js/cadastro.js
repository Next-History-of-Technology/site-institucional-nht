...
function validarEmail() {
  ...
  if (
    email.endsWith("sptech.school") || //Se o email terminar com algumas dessas opções, entre no loop (while)
    email.endsWith(".com") ||
    email.endsWith(".com.br")
  ) {
    while (contador < email.length) {
      //Enquanto o contador for menor que o tamanho do texto
      contador++;

      if (email[contador] == "@") {
        //Percorrendo cada posição do email do usuário para verificar se existe o caractere "@"
...
function validarNome() {
  //Função para verificar se é um nome válido
  ...
  while (contador < nome.length) {
    ...
    if (nome.length <= 1) {
      //Se o tamanho do nome for menor ou igual a 1, o nome é inválido
...
function validarTelefone() {
  //Função que verifica se o telefone é válido
  var telefone = input_telefone.value;
  var contador = 0;

  while (contador < 15) {
    //Enquanto o contador for menor que 15(Número de caracteres máximo do telefone com as formatações)
    contador++;
    if (telefone.length == 15) {
      //Se o tamanho do telefone for igual a 15, ele é válido
  ...
  if (telefoneLength <= telefone.length) {
    if (telefone.length == 2) {
      //Se o tamanho do telefone for igual a dois, adiciona parênteses entre os número
      input_telefone.value = "(" + telefone + ") ";
    } else if (telefone.length == 10) {
      //Se o tamanho do telefone for igual a 10, adiciona um traço antes dos 4 últimos números
...
function validarCpf() {
  ...
  while (contador < 14) {
    //Enquanto o contador for menor que 14(Número máximo de caracteres com a formatação do CPF)
    contador++;

    if (cpf.length == 14) {
      //Se o tamanho do CPF inserido for igual a 14, ele é válido
      cpf_span.innerHTML = "";
  ...
  if (cpfLength <= cpf.length) {
    if (cpf.length == 3 || cpf.length == 7) {
      //Se o tamanho do CPF for igual a 3 ou igual a 7, adiciona um ponto antes dos números seguintes
      input_cpf.value += ".";
    } else if (cpf.length == 11) {
      //Se o tamanho do CPF inserido for igual a 11, adiciona um traço antes dos dígitos finais
      input_cpf.value += "-";
    }
  }
...
function validarSenha() {
  //Função que valida a Senha inserida
  var senha = input_senha.value;

  if (senha.length < 8) {
    //Se o tamanho da senha for menor que 8 caracteres, ela não será válida
...
function validarSenhaRepetida() {
  //Função que valida a senha repetida
  var senha = input_senha.value;
  var senhaRepetida = input_confirmar_senha.value;

  if (senhaRepetida !== senha || senhaRepetida.length < 8) {
    //Se as senhas forem diferentes ou se o tamanho da senha repetida for menor que 8, a senha repetida será inválida
